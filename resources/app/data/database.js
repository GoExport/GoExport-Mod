...
module.exports = class GoDatabase {
  constructor(isSettings = false) {
    if (isSettings) {
      ...
      baseDb = {
        ...
        GE_ASPECT: "16:9", // Sets GoExport aspect ratio to 16:9
    ...
    if (!fs.existsSync(this.path)) {
      ...
      try {
        ...
        throw new Error(
          "Something is extremely awfully horribly terribly preposterously crazily insanely madly wrong. You may be in a read-only system/admin folder."
        );
      }
    }
  }
  ...
  get(from, id) {
    if (!from || !id) {
      throw new Error(
        "Must input a category to select from or an id to look for."
      );
    }
